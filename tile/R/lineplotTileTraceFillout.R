lineplotTileTraceFillout <- function(trace,tracedefault) {
    ...
    trace$ci$mark <- c(trace$ci$mark,fillout("shaded",length(trace$ci$levels)))[1:length(trace$ci$levels)]
    ...
    if (maxtrace) {
        ...
        trace$polyalpha <- fillout(trace$polyalpha,maxtrace)
