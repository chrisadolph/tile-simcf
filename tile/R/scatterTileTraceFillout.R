scatterTileTraceFillout <- function(trace,tracedefault) {
    ...
    if (maxtrace) {
        ...
        if (any(class(trace$size)=="unit")) {
            trace$size <- filloutunits(trace$size,maxtrace)
        } else {
            trace$size <- fillout(trace$size,maxtrace)
            trace$size <- unit(trace$size,"char")
        }
